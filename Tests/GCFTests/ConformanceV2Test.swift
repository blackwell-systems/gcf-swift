...
final class ConformanceV2Test: XCTestCase {
    ...
    private func runFixture(rel: String, op: String, fx: OrderedDictionary) throws {
        ...
        case "graph-stream-encode":
            ...
            let streamOpts = fx["options"] as? OrderedDictionary
            if let so = streamOpts, so.orderedKeys.contains(where: { $0 != "labeledTrailerCounts" }) {
                throw XCTSkip("unsupported stream options")
            }
            let labeledTrailer = (streamOpts?["labeledTrailerCounts"] as? Bool) ?? false
            let expected = fx["expected"] as? String ?? ""
            let payload = toPayload(fx["input"])
            let sink = StringSink()
            let opts = StreamOptions(tokenBudget: payload.tokenBudget,
                                     ...
                                     labeledTrailerCounts: labeledTrailer)
